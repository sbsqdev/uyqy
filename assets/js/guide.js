...
const Guide = (() => {
  ...
  const INTENTS = [
    ...
    },
    {
      ...
      run: (q, c) => {
        const list = REGATTAS.filter(r => r.city === c.dest.id);
        const other = REGATTAS.filter(r => r.city !== c.dest.id);
        const line = r => `• <b>${r.name}</b> — ${r.date}. ${r.fleet}. ${r.slots}. За участие <b>+${r.pts}</b> очков.`;
        return {
          text: (list.length ? `Регаты в городе <b>${c.dest.city}</b>:\n\n${list.map(line).join('\n')}\n\n` : '')
            + `Остальной календарь:\n${other.map(line).join('\n')}\n\nОтметиться можно в разделе «Регаты» — очки упадут в профиль сразу.`,
    ...
    {
      id: 'city',
      test: q => has(q, ['город', 'куда поехат', 'где лучше', 'сравни', 'выбрат']),
      run: (q, c) => ({
        text: `Сейчас на карте три города:\n\n`
          + DESTINATIONS.map(d => `• <b>${d.city}</b> (${d.region}) — ${d.blurb}\n  Ветер: ${d.wind}. ${d.regatta}.`).join('\n\n')
          + `\n\nПереключить город можно вкладками над картой — гид переедет вместе с вами.`,
        chips: ['Собери план на день', 'Календарь регат', 'Что здесь с ветром?']
      })
    }
  ];
